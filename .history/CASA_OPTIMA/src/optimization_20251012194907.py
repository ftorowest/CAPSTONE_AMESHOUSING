...
def optimize_house(
    ...
    "Bsmt_Exposure","TotRms_AbvGrd", "Heating_QC", "Year_Built", "Year_Remod_Add", "Bsmt_Qual", "House_Style_One_Story", "Garage_Finish" }
    ...
    # Variable binaria de existencia de sotano
    B = m.addVar(vtype=GRB.BINARY, name="Basement_Binary")

    # Activación del sótano (si tiene metros cuadrados, existe)
    m.addConstr(x["Total_Bsmt_SF"] <= M_grande * B, name="bsmt_activation_upper")
    m.addConstr(x["Total_Bsmt_SF"] >= 1 * B, name="bsmt_activation_lower")  # al menos 1 pie² si existe

    # Atributos del sótano que solo existen si hay sótano
    basement_related = ["Bsmt_Qual", "Bsmt_Exposure", "Bsmt_Full_Bath"]

    for v in basement_related:
        # Si no hay sótano → atributo = 0
        m.addConstr(x[v] <= M_grande * B, name=f"{v}_upper_if_basement")
        # Si hay sótano → atributo >= 1
        m.addConstr(x[v] >= B, name=f"{v}_lower_if_basement")

    # También se puede mantener la coherencia con el tamaño mínimo de baño en sótano
    m.addConstr(x["Total_Bsmt_SF"] >= baño_promedio * x["Bsmt_Full_Bath"], name="bsmt_bath_min_size")
        ...
        m.addConstr( x["Total_Bsmt_SF"] >= baño_promedio * x["Bsmt_Full_Bath"], name = "bsmt_bath")
