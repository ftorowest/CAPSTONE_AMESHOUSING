...
def optimize_house(
    ...
    m.addConstr( x["First_Flr_SF"] + x["Second_Flr_SF"] + x["Total_Bsmt_SF"] >= x["Full_Bath"] * baño_promedio + x["Kitchen_AbvGr"] * cocina_promedio + x["TotRms_AbvGrd"] * habitacion_promedio , name="sf_min")

    m.addConstrs( x["Kitchen_AbvGr"] <= M_grande * x["Kitchen_Qual"], name = "calidad_cocina")

    m.addConstrs( x["Total_Bsmt_SF"] <= M_grande * x["Bsmt_Exposure"], name = "bsmt_exposure")

    m.addConstrs( x["Total_Bsmt_SF"] <= M_grande * x["Bsmt_Quality"], name = "bsmt_quality")
    
    m.addConstrs( x["Total_Bsmt_SF"] >= baño_promedio * x["Bsmt_Full_Bath"], name = "bsmt_bath")

    m.addConstrs( x["Garage_Cars"] * espacio_por_auto <= M_grande * x["Garage_Finish"], name = "g_finish")

    m.addConstrs( x["Garage_Cars"] * espacio_por_auto <= M_grande * x["Garage_Cond"], name = "g_cond")

    m.addConstrs( x["Garage_Cars"] * espacio_por_auto <= M_grande * x["Garage_Qual"], name = "g_qual")
