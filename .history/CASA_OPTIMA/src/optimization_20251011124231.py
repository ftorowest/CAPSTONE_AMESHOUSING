...
def optimize_house(
    ...
    default_costs = {
        "First_Flr_SF":        800,   # costo por pie² en el primer piso
        "Second_Flr_SF":       820,   # segundo piso es más caro estructuralmente
    ...
    #Variable binaria para activación de restricciones en ampliaciones
    min_delta = {"First_Flr_SF": 40, "Second_Flr_SF": 40, "Total_Bsmt_SF": 20, "Pool_Area": 20}
    A = m.addVar(vtype=GRB.BINARY, name="Ampliacion")
    ampliaciones = {}
    for v, min_inc in min_increase.items():
    # 1️⃣ Crear variable binaria: 1 si hay ampliación significativa, 0 si no se modifica
    y_expand[v] = m.addVar(vtype=GRB.BINARY, name=f"y_expand_{v}")

    # 2️⃣ Si se amplía, debe aumentar al menos min_inc
    m.addConstr(
        x[v] - baseline[v] >= min_inc * y_expand[v],
        name=f"{v}_min_increase"
    )

    # 3️⃣ Si no se amplía, el cambio debe ser 0
    m.addConstr(
        x[v] - baseline[v] <= M_big * y_expand[v],
        name=f"{v}_activation_limit"
    )
