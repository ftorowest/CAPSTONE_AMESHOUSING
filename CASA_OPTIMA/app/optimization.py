...
try:
    ...
    # Si no encuentra el archivo, continuar sin verificación de factibilidad
    def check_house_feasibility(*args, **kwargs):
        return True, {}
...
def optimize_house(
    ...
    # Helper para acceder a baseline de forma segura (Series o DataFrame)
    def get_baseline_value(col):
        if isinstance(baseline, pd.DataFrame):
            return float(baseline[col].iloc[0])
        else:
    ...
    if zero == False:
        ...
        print(f"\nConstruyendo casa desde cero en las coordenadas ({get_baseline_value('Longitude'):.6f}, {get_baseline_value('Latitude'):.6f})")
    ...
    for v, min_d in min_delta.items():
        ...
        m.addConstr( x[v] - get_baseline_value(v) >= min_d * ampliaciones[v], name=f"{v}_min_delta")

        # Si no se amplía, el cambio debe ser 0
        m.addConstr(x[v] - get_baseline_value(v) <= M_grande * ampliaciones[v], name=f"A_{v}_activation")
    ...
    had_basement = 1 if get_baseline_value("Total_Bsmt_SF") > 0 else 0
    ...
    if zero:
        ...
        m.addConstr(
            x["Bsmt_Qual"] == 4 * B_new + get_baseline_value("Bsmt_Qual") * (1 - B_new),
            name="BsmtQual_if_new"
        )

        m.addConstr(
            x["Bsmt_Exposure"] == 1 * B_new + get_baseline_value("Bsmt_Exposure") * (1 - B_new),
            name="BsmtExposure_if_new"
        )

        # Si existía basement antes, permitir mejorar calidad (sí cobra)
        if had_basement == 1:
            m.addConstr(x["Bsmt_Qual"] >= get_baseline_value("Bsmt_Qual"), name="BsmtQual_can_improve")
            m.addConstr(x["Bsmt_Exposure"] >= get_baseline_value("Bsmt_Exposure"), name="BsmtExposure_can_improve")
    ...
    had_garage = 1 if get_baseline_value("Garage_Area") > 0 else 0
    ...
    if zero:
        ...
        m.addConstr(
            x["Garage_Cond"] == 4 * G_new + get_baseline_value("Garage_Cond") * (1 - G_new),
            name="GarageCond_if_new"
        )

        m.addConstr(
            x["Garage_Qual"] == 4 * G_new + get_baseline_value("Garage_Qual") * (1 - G_new),
            name="GarageQual_if_new"
        )

        m.addConstr(
            x["Garage_Finish"] == 4 * G_new + get_baseline_value("Garage_Finish") * (1 - G_new),
    ...
    if zero == False:
        baseline_quality_avg = (
            get_baseline_value("Exter_Qual") +
            get_baseline_value("Kitchen_Qual") +
            get_baseline_value("Bsmt_Qual") +
            get_baseline_value("Heating_QC") +
            get_baseline_value("Garage_Cond") +
            get_baseline_value("Garage_Qual")) / 6
        # Variable auxiliar
        ...
        m.addConstr(x["Overall_Cond"] >= get_baseline_value("Overall_Cond"))
    ...
    if zero == True:
        price_before = land_price if land_price is not None else 0
    ...
    if m.SolCount > 0:
        ...
        # Acceder correctamente a baseline según su tipo (DataFrame o Series)
        if isinstance(baseline, pd.DataFrame):
            deltas = {c: x[c].X - float(baseline[c].iloc[0]) for c in trained_feats}
        else:
