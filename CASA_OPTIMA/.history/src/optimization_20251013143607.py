...
def optimize_house(
    ...
    default_costs = {
        ...
        "Bsmt_Qual":           20 * sotano_promedio,  # mejorar calidad del sótano (por nivel)
    ...
    area_inicial_garage = float(baseline["Garage_Cars"]) * espacio_por_auto

    c_garage_qual = default_costs["Garage_Qual"]
    cost_garage_qual = c_garage_qual * area_inicial_garage * (x["Garage_Qual"] - float(baseline["Garage_Qual"]))

    c_garage_cond = default_costs["Garage_Cond"]
    cost_garage_cond = c_garage_cond * area_inicial_garage * (x["Garage_Cond"] - float(baseline["Garage_Cond"]))

    c_garage_finish = default_costs["Garage_Finish"]
    cost_garage_finish = c_garage_finish * area_inicial_garage * (x["Garage_Finish"] - float(baseline["Garage_Finish"]))

    #Calcular Costos Basement
    area_inicial_basement = float(baseline["Total_Bsmt_SF"]) 

    c_basemente_qual = default_costs["Garage_Qual"]
