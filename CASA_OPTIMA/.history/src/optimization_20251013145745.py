...
def optimize_house(
    ...
    if m.SolCount > 0:
        ...
        # === 1️⃣ Variables con costo estándar lineal ===
        for c in trained_feats:
            if c in ["Garage_Qual", "Garage_Cond", "Garage_Finish", "Bsmt_Qual"]:
                continue  # estas se tratan aparte
        ...
        # === 2️⃣ Costos dependientes de área ===
        ...
        if abs(delta_garage_qual) > 1e-6:
            ...
            print(f"{'Garage_Qual (area)':25s} {delta_garage_qual:+10.3f} {c_garage_qual*area_garage:15,.0f} {total_garage_qual:15,.0f}")
        if abs(delta_garage_cond) > 1e-6:
            cost_breakdown["Garage_Cond"] = total_garage_cond
            print(f"{'Garage_Cond (area)':25s} {delta_garage_cond:+10.3f} {c_garage_cond*area_garage:15,.0f} {total_garage_cond:15,.0f}")
        if abs(delta_garage_finish) > 1e-6:
            cost_breakdown["Garage_Finish"] = total_garage_finish
            print(f"{'Garage_Finish (area)':25s} {delta_garage_finish:+10.3f} {c_garage_finish*area_garage:15,.0f} {total_garage_finish:15,.0f}")
        ...
        if abs(delta_basement_qual) > 1e-6:
            ...
            print(f"{'Bsmt_Qual (area)':25s} {delta_basement_qual:+10.3f} {c_basement_qual*area_basement:15,.0f} {total_basement_qual:15,.0f}")
