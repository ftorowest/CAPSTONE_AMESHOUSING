...
def optimize_house(
    ...
    cost_garage_qual = c_garage_qual * area_garage * (x["Garage_Qual"] - float(baseline["Garage_Qual"]))

    c_garage_cond = default_costs["Garage_Cond"]
    cost_garage_cond = c_garage_cond * area_garage * (x["Garage_Cond"] - float(baseline["Garage_Cond"]))

    c_garage_finish = default_costs["Garage_Finish"]
    cost_garage_finish = c_garage_finish * area_garage * (x["Garage_Finish"] - float(baseline["Garage_Finish"]))

    #Calcular Costos Basement
    area_basement = float(baseline["Total_Bsmt_SF"]) 

    c_basement_qual = default_costs["Bsmt_Qual"]
    cost_basement_qual = c_basement_qual * area_basement * (x["Bsmt_Qual"] - float(baseline["Bsmt_Qual"]))


    # Restricciones básicas

    # 1. Restricción de presupuesto total
    cost_expr = (cost_standard + cost_garage_qual + cost_garage_cond + cost_garage_finish + cost_basement_qual)
